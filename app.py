...
st.write("希望大家能花一分鐘為我們下學年的團長投下神聖的一票。本系統採用『匿名設計』，可以安心投票。")
st.markdown("---")

# 1. 身份驗證區
st.header("✨ 夥伴，請先驗證身份")

st.info("""🔒 **關於投票隱私，請大家放心：**
* **填寫真實姓名僅作為「驗證簽到」使用，目的是為了避免重複投票。**
* 系統後台只會單獨紀錄誰已經簽到投票，與您投下的「同意/不同意」完全分開儲存，**後台絕對看不到是誰投給誰**，請大家放心真誠地投票唷！
...
with st.expander("📊 幹部專區：密碼驗證解鎖 (點擊展開)"):
    ...
    if input_pwd == ADMIN_PASSWORD:
        ...
        if fix_name:
            if fix_name not in data["voted_status"]:
                st.error("找不到這位夥伴的名字，請確認是否打錯字。")
            elif not data["voted_status"][fix_name]:
